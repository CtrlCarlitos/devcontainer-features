...
|---------|-------------|-------------|
| `runtime_core` | Installs Node.js runtime (nvm) + foundational tools | - |
...
| `curated-skills` | 16 curated agent skills via the `skills` CLI — matches the CtrlCarlitos dotfiles set; supports `skill>local-name` renames for collision dodging | 18+ |
...
The `runtime_core` feature defaults to **v22**, which satisfies all feature requirements:
...
{
  "features": {
    "ghcr.io/ctrlcarlitos/devcontainer-features/runtime_core:1": { "version": "22" },
